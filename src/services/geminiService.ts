...
/**
 * Generates a simple list of emotions/keywords for the Upload Sheet mode.
 * Format: "Happy, Sad, Angry, ..."
 */
export const generateSimpleIcons = async (
    qty: number,
    topic: string
): Promise<string> => {
    const ai = getAI();
    const prompt = `
    Generate a list of exactly ${qty} distinct emotions, expressions, or simple keywords related to the topic: "${topic}".
    ...
    const candidate = validateResponse(response);
