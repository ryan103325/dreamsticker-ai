...
export const WorksGallery: React.FC<WorksGalleryProps> = ({ works, isDark, t, onRestore, onDelete, onClearAll, onClose }) => {
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <div
                className={`w-full max-w-3xl max-h-[85vh] rounded-3xl shadow-2xl border flex flex-col ${isDark ? 'bg-slate-900 border-white/10 text-white' : 'bg-white border-slate-200 text-slate-800'}`}
            ...
            >
                <div className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={onClose} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold transition-colors ${isDark ? 'hover:bg-white/10' : 'hover:bg-slate-100'}`}>✕</button>
                    </div>
                </div>
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <div className="flex gap-1.5 shrink-0">
                                            ...
                                            <button
                                                ...
                                                className={`px-2 py-1.5 text-xs font-bold rounded-lg transition-colors ${isDark ? 'text-red-300 hover:bg-red-500/20' : 'text-red-400 hover:bg-red-50'}`}
